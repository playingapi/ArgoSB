...
echo "ArgoSB真一键无交互脚本（GitHub Actions 优化版）"
...
apt update -y
apt install curl wget tar gzip jq -y
elif command -v yum &> /dev/null; then
yum install -y curl wget jq tar
elif command -v apk &> /dev/null; then
apk update -y
apk add wget curl tar jq tzdata openssl git grep
else
...
2、2096端口的vmess-ws-tls-argo节点，默认优选IPV6：[2606:4700::]
$line6

3、80端口的vmess-ws-argo节点，默认优选IPV4：104.21.0.0
$line7

4、2095端口的vmess-ws-argo节点，默认优选IPV6：[2400:cb00:2049::]
$line13
...
相关快捷方式如下：
显示域名及节点信息：cat /etc/s-box-ag/list.txt
---------------------------------------------------------
EOF
